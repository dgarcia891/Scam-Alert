...
| BUG-066 | CLOSED | Critical | Ghost Badge on ALL pages — `overallSeverity` always `undefined` | Fixed `createScanResult` in `scan-schema.js` to expose `overallSeverity` (alias for `severity`) and `overallThreat` (computed boolean). `undefined !== 'SAFE'` was always `true`, triggering the badge on every page. |
