...
import { checkSuspiciousPort } from '../../src/lib/analyzer/url-engine.js';

describe('BUG-083: False Negative on "Nostalgic Photos" Scam', () => {
    const SCAM_URL = 'https://tkygoc.rdlxlcewc.com:8443/';

    test('checkSuspiciousPort should flag non-standard port :8443', () => {
    ...
    test('Standard HTTPS port 443 should NOT be flagged', () => {
        const result = checkSuspiciousPort('https://google.com/');
        expect(result.flagged).toBe(false);
    });

    test('checkEmailScams should flag vague lure keyword + external link', () => {
        const pageContent = {
            isEmailView: true,
            emailContext: true,
    ...
    test('analyzeUrl should produce a non-zero risk score for the scam URL', () => {
        const result = analyzeUrl(SCAM_URL, null, true);
